...
export class ImprovementProposer {
  propose(run: TaskRun): ImprovementProposal | undefined {
    if (run.status !== "failed" && run.status !== "blocked") return undefined;
    ...
    return {
      ...
      title: noSteps
        ? "Add capability coverage for an unmatched task"
        : "Investigate a failed routed execution",
      rationale: noSteps
      ...
      expectedBenefit:
        "Reduce repeated routing failures while preserving explicit review and tests.",
