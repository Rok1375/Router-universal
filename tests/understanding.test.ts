...
describe("RuleBasedTaskAnalyzer", () => {
  it("classifies a software creation request and its permissions", async () => {
    const request = TaskRequestSchema.parse({ prompt: "Create a React project, run tests, and push it to GitHub." });
    ...
    expect(result.requiredPermissions).toEqual(
      expect.arrayContaining(["filesystem:read", "filesystem:write", "process:run", "network:access"]),
    );
