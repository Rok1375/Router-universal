...
import { errorMessage } from "../../../packages/core/src/errors.js";
...
export async function buildServer(runtime?: Awaited<ReturnType<typeof createRuntime>>) {
  ...
  app.post("/v1/routes/preview", async (request, reply) => {
    ...
    if (!task.success) return reply.code(400).send({ error: "invalid-task", issues: task.error.issues });
    return activeRuntime.router.preview(task.data);
  });

  app.post("/v1/tasks", async (request, reply) => {
    const body = RunBodySchema.safeParse(request.body);
    if (!body.success) return reply.code(400).send({ error: "invalid-run", issues: body.error.issues });
    return activeRuntime.router.run(body.data.task, body.data.approvals);
  });

  app.setErrorHandler((error, _request, reply) => {
    app.log.error({ err: error }, "request failed");
    return reply.code(500).send({ error: "internal-error", message: errorMessage(error) });
  });
